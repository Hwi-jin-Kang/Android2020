...
### 한눈에 살펴보기
![화면 구성](https://user-images.githubusercontent.com/71058308/104150928-78f0e900-5416-11eb-8995-b7fe1b93620a.jpg)

* 이름, 번호, 이메일, 그룹에 대한 데이터 값을 받은 후 DB에 저장
* 동명이인이 있을 경우를 생각해 번호에 대한 데이터 값을 Primary Key로 지정
* 이메일, 그룹에 대해서는 디비에서 Text로 설정해서 데이터를 입력하지 않아도 DB에 저장되게 설정
--------------------------------------------------------------------------------------------------------------
* '목록보기' 버튼을 누른 후 자동으로 읽히는게 아닌 '불러오기'버튼을 눌러야만 DB의 데이터를 받아오는 불편함이 있음
* 수정, 삭제하는 부분에 있어서 초기화면으로 돌아가 번호를 무조건 입력해야 실행이 됨
--------------------------------------------------------------------------------------------------------------
